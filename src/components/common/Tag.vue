...
import {swiper,swiperSlide} from 'vue-awesome-swiper'
	export default({
		...
		created() {
			...
			this.preloadimages(['static/images/Cards_btn_grey.png','static/images/Cards_back.png'])
		},
		updated() {
			this.getImgEl()
		},
		methods: {
			...
			flipCard(index,id,money,imgUrl) {
				...
				if(index===4){
					...
					if (this.flag) {
						let random = Math.floor(Math.random()*8)//后台传入随机数8就是0金币
...
<style>
	.jump {
		-webkit-animation: myJump 2s;
		-ms-animation: myJump 2s;
		animation: myJump 2s;
	}
	@keyframes myJump{
		0% {
	...
	.emoji {
		...
		-webkit-animation: myEmoji 2s cubic-bezier(0.49, -0.29, 0.75, 0.41);
		-ms-animation: myEmoji 2s cubic-bezier(0.49, -0.29, 0.75, 0.41);
	}
	@keyframes myEmoji{
		0% {
	...
	.myContainer {
		...
		-webkit-box-shadow: 0 0 16px #45dee0;
	}
	.swiper-slide {
		padding-top: 60px;
	}
	.card-btn {
		-webkit-animation: clickbtn 0.2s;
		-ms-animation: clickbtn 0.2s;
